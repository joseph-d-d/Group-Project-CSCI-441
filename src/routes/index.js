...
router.get(["/forgot", "/forgot.html"], function (req, res) {
  res.sendFile(
    path.join(__dirname, "../../../Group-Project-CSCI-441-Frontend/forgot.html")
  );
});
...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      const user = (
        await req.app.locals.user.setUserResetPasswordToken(
          req.body.email,
          token,
          tokenExpires
        )
...
router.get("/reset/:token", async function (req, res) {
  ...
  const foundUser = await req.app.locals.user.getUserByToken(req.params.token);
...
router.post("/reset/:token", async function (req, res) {
  ...
  const foundUser = (
    await req.app.locals.user.unsetTokenAndUpdatePassword(
      req.params.token,
      req.body.password
    )
  ).value;
